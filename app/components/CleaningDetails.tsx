...
export function ServiceOverview() {
  ...
  return <div className="home-services">{services.map((s,i) => <article className={i === 1 ? "home-service featured" : "home-service"} key={s.key}>
    ...
    <div className="service-links"><a href="#calculator" onClick={() => update({ service: s.key })}>Рассчитать <span aria-hidden="true">↗</span></a><Link href={s.href}>Подробнее</Link></div>
  </article>)}</div>;
}
...
export function CleaningDetails() {
  ...
  return <div className="checklist-layout"><div className="checklist-intro"><span className="checklist-symbol" aria-hidden="true">✳</span><h3>{pricing[input.service].label}<br />уборка</h3><p>Состав зависит от выбранного типа уборки. Деликатные материалы и сложные загрязнения обсудим заранее.</p><a className="text-link" href="#calculator">Изменить тип уборки ↗</a></div><div className="checklist-content"><div className="room-tabs" role="tablist" aria-label="Зона уборки">{rooms.map((r,i) => <button type="button" key={r} role="tab" id={`room-tab-${i}`} aria-controls="room-panel" aria-selected={room === i} tabIndex={room === i ? 0 : -1} onClick={() => setRoom(i)} onKeyDown={e => { if (["ArrowLeft", "ArrowRight", "Home", "End"].includes(e.key)) { e.preventDefault(); const next = e.key === "Home" ? 0 : e.key === "End" ? 2 : (room + (e.key === "ArrowRight" ? 1 : 2)) % 3; setRoom(next); document.getElementById(`room-tab-${next}`)?.focus(); } }}>{r}</button>)}</div><div role="tabpanel" id="room-panel" aria-labelledby={`room-tab-${room}`}><ul>{list.map(item => <li key={item}><span aria-hidden="true">✓</span>{item}</li>)}</ul></div><p className="checklist-note">Отдельно: окна, техника и шкафы внутри, балкон, глажка. Выберите их в калькуляторе.</p></div></div>;
}
